# FROM https://github.com/NixOS/nixpkgs/pull/326699
{ lib, stdenv, config, alsa-lib, cmake, darwin, dbus, fcitx5, fetchFromGitHub
, ibus, installShellFiles, libGL, libdecor, libdrm, libjack2, libpulseaudio
, libusb1, libxkbcommon, mesa, nas, ninja, nix-update-script, nixosTests
, pipewire, pkg-config, sndio, systemdLibs, testers, validatePkgConfig, wayland
, wayland-scanner, xorg, zenity
...
stdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [ cmake ninja validatePkgConfig ]
    ++ lib.optional waylandSupport wayland-scanner;

  buildInputs = finalAttrs.dlopenBuildInputs
    ++ lib.optionals stdenv.hostPlatform.isDarwin (with darwin.apple_sdk_11_0; [
      frameworks.AVFoundation
      frameworks.Cocoa
      frameworks.ForceFeedback
      libs.xpc
    ]) ++ lib.optionals ibusSupport [ fcitx5 ibus ]
    ++ lib.optional waylandSupport zenity;

  dlopenBuildInputs = lib.optionals stdenv.hostPlatform.isLinux [
    libusb1
    nas # libaudio
    ...
    ++ lib.optional openglSupport libGL ++ lib.optional pipewireSupport pipewire
  ...
  env = {
    ...
    NIX_LDFLAGS =
      ...
      "-rpath ${
        lib.makeLibraryPath
        (finalAttrs.dlopenBuildInputs ++ finalAttrs.dlopenPropagatedBuildInputs)
      }";
  };

  passthru = {
    # Building this in its own derivation to make sure the rpath hack above propagate to users
    snake-example = stdenv.mkDerivation (finalAttrs': {
      pname = "sdl3-snake-example";
      inherit (finalAttrs) version src;

      sourceRoot = "${finalAttrs'.src.name}/examples/game/01-snake";
      ...
      postConfigure = ''
        appendToVar NIX_CFLAGS_COMPILE "$(pkg-config sdl3 --libs-only-l)"
      '';
    ...
    tests = {
      ...
      inherit (finalAttrs.passthru) snake-example;
    };

    updateScript =
      nix-update-script { extraArgs = [ "--version-regex" "'preview-(.*)'" ]; };
  };
